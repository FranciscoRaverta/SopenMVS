...
int main(int argc, LPCTSTR* argv)
{
	...
	if (!OPT::strSegmentationPath.empty() and OPTDENSE::nEstimateSegmentations == 2) {
		...
		for (Image& image : scene.images) {
			...
			image.segmentationName = OPT::strSegmentationPath + Util::getFileName(image.name) + ".tif.png";
	...
	if (!OPT::strUncertaintyPath.empty() and OPTDENSE::nEstimateSegmentations == 2) {
		...
		for (Image& image : scene.images) {
			...
			image.uncertaintyName = OPT::strUncertaintyPath + Util::getFileName(image.name) + ".tif.npz";
	...
	if (!OPT::strProbabilitiesPath.empty() and OPTDENSE::nEstimateSegmentations == 2) {
		...
		for (Image& image : scene.images) {
			...
			image.probabilitiesName = OPT::strProbabilitiesPath + Util::getFileName(image.name) + ".tif.npz";
