	...
	FOREACHPTR(pConnection, connections) {
		...
		for (int i=0; i<sizeMap.height; ++i) {
			for (int j=0; j<sizeMap.width; ++j) {
				...
				SEACAVE::Matrix3x3d poseCovariance = (PoseCovarianceEstimation(imageData.camera, depth, Point2f(x), C_pose).inv())*REAL(REAL(1)/confidence)*REAL(REAL(1)/confidence);
				...
				for (const ViewScore& neighbor: depthData.neighbors) {
					...
					if (IsDepthSimilar(pt.z, depthB, OPTDENSE::fDepthDiffThreshold)) {
						...
						if (normal.dot(normalB) > normalError) {
							...
							poseCovariance += (PoseCovarianceEstimation(imageDataB.camera, depthB, Point2f(xB), C_poseB).inv())*REAL(REAL(1)/confidenceB)*REAL(REAL(1)/confidenceB);
